...
const Schema = z.object({
  ...
  preferences: z.string().optional().transform((s) =>
    typeof s === "string" && s.trim()
      ? s.split(",").map(t => t.trim()).filter(Boolean)
      : []
  ),
  dislikes: z.string().optional().transform((s) =>
    typeof s === "string" && s.trim()
      ? s.split(",").map(t => t.trim()).filter(Boolean)
      : []
  ),
  allergies: z.string().optional().transform((s) =>
    typeof s === "string" && s.trim()
      ? s.split(",").map(t => t.trim()).filter(Boolean)
      : []
  ),
  dietaryPattern: z.enum(["onívoro", "vegetariano", "vegano", "pescetariano", "outro"]),
  mealsPerDay: z.coerce.number().min(1).max(8),
  targetCalories: z.coerce.number().min(800).max(6000),
...
export function NutritionWizard({ onSubmit }: { onSubmit: (prefs: NutritionPrefs) => void }) {
  ...
  const handleSubmit = (values: FormValues) => {
    toast({ title: "Gerando plano…", description: "Isso leva só alguns segundos." })
    onSubmit(values as NutritionPrefs)
  }
    ...
    <form onSubmit={form.handleSubmit(()=>handleSubmit)} className="grid gap-4">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <Select defaultValue={form.getValues("goal")} onValueChange={(v) => form.setValue("goal", v as any)}>
        ...
        <div>
          ...
          <Select defaultValue={form.getValues("dietaryPattern")} onValueChange={(v) => form.setValue("dietaryPattern", v as any)}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <Select defaultValue={form.getValues("budget")} onValueChange={(v) => form.setValue("budget", v as any)}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div>
          ...
          <Select defaultValue={form.getValues("cookingSkill")} onValueChange={(v) => form.setValue("cookingSkill", v as any)}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <Select defaultValue={form.getValues("timePerMeal")} onValueChange={(v) => form.setValue("timePerMeal", v as any)}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
