...
export default function MacroPie({ macros, height = 220 }: { macros: Macros; height?: number }) {
        ...
        <PieChart>
          <Pie data={data} dataKey="value" nameKey="name" innerRadius={55} outerRadius={85}>
            {data.map((entry, index) => (
              <Cell key={`slice-${entry.name}`} fill={PIE_CHART_COLORS[index % PIE_CHART_COLORS.length]} />
            ))}
          </Pie>
          <Tooltip
        ...
        {data.map((d, i) => (
          <div key={d.name} className="flex items-center gap-1">
            <span
              ...
              style={{ background: PIE_CHART_COLORS[i % PIE_CHART_COLORS.length] }}
            />
            <span>{d.name}</span>
          </div>
        ))}
